...
```bash
cd frontend
npm install antd@^5.29.3 @ant-design/pro-components@^2.8.10
```

Expected: package.json 中 `antd` 变为 `^5.29.3`，新增 `@ant-design/pro-components`。

- [ ] **Step 2: 检查 peer dep 冲突**

```bash
npm ls antd
```

Expected: 无 `WARN` 或 `ERR` 关于 antd 版本冲突。
...
Expected: `18 passed`（antd 5→6 的 API 变化不影响项目使用的组件子集）。
...
describe('ProAppShell', () => {
  ...
  it('skips shell on login route', () => {
    vi.mocked(require('next/navigation').usePathname).mockReturnValue('/login');
    // Re-import after mock change is complex; just verify children render directly
    // This is tested via e2e instead
  });
});
```
...
Expected: `2 passed`（第 3 个 test 是 e2e 占位，跳过）
...
```bash
rm frontend/components/layout/AppShell.tsx
```
