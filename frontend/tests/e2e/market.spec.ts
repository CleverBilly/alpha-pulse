...
test("market page shows overview, price ladder and signal tape", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "微结构时间线" })).toBeVisible();
  await expect(page.getByTestId("market-diagnostics-strip").getByRole("heading", { name: "订单流" })).toBeVisible();
  await expect(page.getByTestId("market-diagnostics-strip").getByRole("heading", { name: "流动性" })).toBeVisible();
  await expect(page.getByRole("heading", { name: "微结构事件" })).toBeVisible();
